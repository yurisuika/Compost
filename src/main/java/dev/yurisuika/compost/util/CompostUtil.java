...
public class CompostUtil {
    ...
    public static void checkBounds() {
        Arrays.stream(config.levels).forEach(level -> Arrays.stream(level.items).forEach(item -> {
            int maxCount = (item.name.contains("{") ? Registry.ITEM.get(new Identifier(item.name.substring(0, item.name.indexOf("{")))) : Registry.ITEM.get(new Identifier(item.name))).getMaxCount();
    ...
    public static ItemStack createItemStack(Config.Level.Item item) {
        try {
            return new ItemStackArgumentType(new CommandRegistryAccess(DynamicRegistryManager.BUILTIN.get())).parse(new StringReader(item.name)).createStack(ThreadLocalRandom.current().nextInt(item.min, item.max + 1), true);
