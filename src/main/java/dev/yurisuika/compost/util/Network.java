        ...
        if (!level.isClientSide()) {
            ...
            Option.getWorlds().forEach(world -> {
                if (Objects.equals(world.getName(), level.getServer().getWorldData().getLevelName())) {
                    world.getProduce().forEach(produce -> ServerPlayNetworking.send((ServerPlayer) player, new ClientboundProducePacket(produce.getItem(), produce.getChance(), produce.getMin(), produce.getMax())));
                }
            });
        }
    }
