    ...
    public static void sendCompositions(Level level, Player player) {
        if (!level.isClientSide()) {
            ...
            Option.getCompositions().forEach((name, composition) -> {
                ClientboundCompostPacket.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) player), new ClientboundCompostPacket(name, composition.getCompost().getItem(), composition.getCompost().getChance(), composition.getCompost().getCount().getMin(), composition.getCompost().getCount().getMax()));
                composition.getWorlds().forEach(world -> ClientboundWorldPacket.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) player), new ClientboundWorldPacket(name, world)));
