...
public class CompositionArgument implements ArgumentType<String> {
    ...
    public String parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (Network.getNetworkCompositions().containsKey(name)) {
            ...
            throw new DynamicCommandExceptionType(object -> new TranslatableComponent("commands.compost.composition.unknown", object)).createWithContext(reader, name);
        }
    }

    @Override
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> commandContext, SuggestionsBuilder suggestionsBuilder) {
        return SharedSuggestionProvider.suggest(Network.getNetworkCompositions().keySet().stream().collect(Collectors.toList()), suggestionsBuilder);
    }
