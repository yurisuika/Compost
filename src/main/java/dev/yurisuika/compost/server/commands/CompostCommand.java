...
public class CompostCommand {
    ...
    public static int reloadConfig(CommandSourceStack source) {
        ...
        source.sendSuccess(new TranslatableComponent("commands.compost.config.reload"), true);
        return 1;
    }
    ...
    public static int resetConfig(CommandSourceStack source) {
        ...
        sendCompositions(source);
        source.sendSuccess(new TranslatableComponent("commands.compost.config.reset"), true);
        return 1;
    }
    ...
    public static int addComposition(CommandSourceStack source, String name, ItemInput item, double chance, int min, int max, String world) {
        Composition composition = new Composition(new Composition.Compost(parseItem(item), chance, new Composition.Compost.Count(min, max)), new HashSet<String>() {{ if (world != null) add(world); }});
    ...
    public static int modifyCompositionCompostItem(CommandSourceStack source, String name, ItemInput item) {
        Configure.setItem(name, parseItem(item));
    ...
    public static String parseItem(ItemInput item) {
        StringBuilder stringBuilder = new StringBuilder(item.getItem().getDescriptionId().replace("item.", "").replace("block.", "").replace(".", ":"));
        if (((ItemInputAccessor) item).getTag() != null) {
            stringBuilder.append(((ItemInputAccessor) item).getTag());
        }
