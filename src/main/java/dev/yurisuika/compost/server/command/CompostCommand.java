    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, boolean dedicated) {
        dispatcher.register(literal("compost")
                .then(literal("config")
                        ...
                        .then(literal("reload")
                                .executes(context -> {
                                    ...
                                    CompostUtil.checkLevels(context.getSource().getMinecraftServer());
                ...
                .then(literal("items")
                        ...
                        .then(literal("query")
                                .executes(context -> {
                                    Config.Level level = CompostUtil.getLevel(context.getSource().getMinecraftServer().getSaveProperties().getLevelName());
                                                ...
                                                .then(argument("min", IntegerArgumentType.integer(0, 64))
                                                        .then(argument("max", IntegerArgumentType.integer(1, 64))
                                                                .executes(context -> {
                                                                    ...
                                                                    CompostUtil.addItem(CompostUtil.getLevel(context.getSource().getMinecraftServer().getSaveProperties().getLevelName()).name, new Config.Level.Item(stringBuilder.toString(), chance, min, max));
                                                                    for (ServerPlayerEntity player : context.getSource().getMinecraftServer().getPlayerManager().getPlayerList()) {
                                                                        NetworkUtil.sendItems(context.getSource().getWorld(), player);
                                                                    }
                        ...
                        .then(literal("remove")
                                .then(argument("item", IntegerArgumentType.integer(1))
                                        .executes(context -> {
                                            Config.Level level = CompostUtil.getLevel(context.getSource().getMinecraftServer().getSaveProperties().getLevelName());
                                            ...
                                            if (index > level.items.length) {
                                                ...
                                                CompostUtil.removeItem(level.name, index);
                                                for (ServerPlayerEntity player : context.getSource().getMinecraftServer().getPlayerManager().getPlayerList()) {
                                                    NetworkUtil.sendItems(context.getSource().getWorld(), player);
                                                }
