        ...
        dispatcher.register(literal("compost")
                .then(literal("config")
                        ...
                        .then(literal("reload")
                                .executes(context -> {
                                    loadConfig();
                                    checkWorlds(context.getSource().getMinecraftServer());
                ...
                .then(literal("groups")
                                                ...
                                                .then(argument("min", IntegerArgumentType.integer(0, 64))
                                                        .then(argument("max", IntegerArgumentType.integer(1, 64))
                                                                .executes(context -> {
                                                                    ...
                                                                    addGroup(config.worlds[getIndex(context.getSource().getMinecraftServer().getSaveProperties().getLevelName()).get()].world, stringBuilder.toString(), chance, min, max);
                                                                    for (ServerPlayerEntity player : context.getSource().getMinecraftServer().getPlayerManager().getPlayerList()) {
                                                                        sendGroups(context.getSource().getWorld(), player);
                                                                    }
                                                                    context.getSource().sendFeedback(new TranslatableText("commands.compost.groups.add", new DecimalFormat("0.###############").format(BigDecimal.valueOf(chance).multiply(BigDecimal.valueOf(100))), (min != max ? min + "-" + max : String.valueOf(max)), itemStack.toHoverableText()), true);
                                                                    return 1;
                                                                })
                                                        )
                                                )
                        ...
                        .then(literal("remove")
                                .then(argument("group", IntegerArgumentType.integer(1))
                                        .executes(context -> {
                                            AtomicInteger index = getIndex(context.getSource().getMinecraftServer().getSaveProperties().getLevelName());
                                            ...
                                            if (range > config.worlds[index.get()].items.length) {
                                                context.getSource().sendError(new TranslatableText("commands.compost.groups.remove.failed", range, world.items.length));
                                                return 0;
                                            } else {
                                                ...
                                                context.getSource().sendFeedback(new TranslatableText("commands.compost.groups.remove", item), true);
                                                return 1;
                                            }
                                        })
                                )
