    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess, RegistrationEnvironment environment) {
        dispatcher.register(literal("compost")
                ...
                .then(literal("items")
                        ...
                        .then(literal("add")
                                .then(argument("name", ItemStackArgumentType.itemStack(registryAccess))
                                                ...
                                                .then(argument("min", IntegerArgumentType.integer(0, 64))
                                                        .then(argument("max", IntegerArgumentType.integer(1, 64))
                                                                .executes(context -> {
                                                                    ...
                                                                    CompostUtil.addItem(CompostUtil.getLevel(context.getSource().getServer().getSaveProperties().getLevelName()).name, new Config.Level.Item(arg.asString(), chance, min, max));
                                                                    for (ServerPlayerEntity player : context.getSource().getServer().getPlayerManager().getPlayerList()) {
                                                                        NetworkUtil.sendItems(context.getSource().getWorld(), player);
                                                                    }
